...
public class StudentAction extends ActionSupport {
	...
	public String addChart() {
		try {
			...
			List<Grade> gradeList = gradeBiz.findGradeBySubIdAndStudent(subid,
					student);

			Integer length = gradeBiz
					.findGradeBySubIdAndStudent(subid, student).size();

			int flag = 0;
			if (length.equals(1)) {
				// length==1,说明考了一次
				ds.addValue(gradeList.get(0).getScore(), gradeList.get(0)
						.getSubject().getSname(), "第" + 1 + "次考试");
			} else {
				// 考了多次
				for (int i = 0; i < length; i++) {
					ds.addValue(gradeList.get(i).getScore(), gradeList.get(i)
							.getSubject().getSname(), "第" + (i + 1) + "次考试");
				}
			}
			...
			String title = "考试成绩图表";
			JFreeChart chart = ChartFactory.createBarChart3D(title, "考试次数",
					"分数", ds, PlotOrientation.VERTICAL, true, false, false);
			...
			plot.getDomainAxis().setTickLabelFont(
					new Font("宋体", Font.PLAIN, 15));// 小标签字体
	...
	public void fy() {
		try {
			...
			if (searchName != null) {
				...
				stuList = studentBiz.findStudentByPageAndStu(nowpage,
						temp_student);
	...
	public String update() {
		try {
			Map session = ActionContext.getContext().getSession();
			session.put("ret", "student/setting.jsp");
			...
			session.put("setting_Info", "<script>alert('修改成功');</script>");
	...
	public String add() {
		try {
			...
			// 首相判断用户名 和登录名是否重复
			Student temp_student1 = studentBiz
					.findStudentByLoginName(loginname);
