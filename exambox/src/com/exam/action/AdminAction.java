...
public class AdminAction extends ActionSupport {
	...
	public String login() {
		try {
			...
			Admin admin2 = adminBiz.login(loginName, loginPwd);
			if (admin != null) {
	...
	public String add() {
		...
		try {
			...
			// 将权限字符串转数组转换为字符串
			String temp_power = "";
			for (String temp : power) {
				temp_power = temp_power + temp + ",";
			}
			temp_admin.setPower(temp_power);
	...
	public String findByPage() {
		...
		try {
			...
			if (adminName != null) {
				...
				adminList = adminBiz.findAdminByPageAndAdmin(nowpage,
						temp_admin);
