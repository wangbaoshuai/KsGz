...
public class QuestionAction extends ActionSupport {
	...
	public String find() {
		try {
			...
			if (nowpage > allPage)
				nowpage = allPage;
			List<Question> questionList = questionBiz.findQuestion(pid,
					nowpage, subid);
			session.put("questionList", questionList);
	...
	public String batch() {
		try {
			if (xls != null && !"".equals(xls)) {
				...
				// String path
				// =ServletActionContext.getRequest().getRealPath("/XLS/"+xlsFileName);
				...
				for (int j = 1; j < sheet.getRows(); j++) {
					...
					q.setDifficulty(Integer.valueOf(sheet.getCell(7, j)
							.getContents()));
					;
