...
public class ErrorAction extends ActionSupport {
	...
	public String find() {
		try {
			...
			List<Error> errorList = errorBiz.findErrorByPageAndStu(nowpage,
					stu, temp_error);
	...
	public String findByPage() {
		try {
			...
			if(session.get("errorSubId")!=null){
				session.remove("errorSubId");
				session.put("errorSubId", subject.getId());
			}else{
				session.put("errorSubId", subject.getId());
			}
			...
			if (temp_subject.getId() != -1) {
				...
				errorList = errorBiz.findErrorByPageAndStu(nowpage, stu,
						temp_error);
				pages = errorBiz.findPageByStu(stu, temp_error);
			} else {
