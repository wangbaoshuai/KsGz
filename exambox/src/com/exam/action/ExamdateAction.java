...
public class ExamdateAction extends ActionSupport {
	...
	public String find() throws Exception {
		try {
			...
			// }
			// //去除重复
			// Set edset=new HashSet<Examdate>();
			// edset.addAll(examdateList);
			// List<Examdate> edlist =new ArrayList<Examdate>();
			// edlist.addAll(edset);
	...
	public String findByPageSubject() throws Exception {
		try {
			...
			Map<String, Object> session = ActionContext.getContext()
					.getSession();
			List<Examdate> examdateList = examdateBiz.findDateByPageAndSubject(
					nowpage, subject);
			if (examdateList.size() > 0) {
				session.remove(examdateList);
				session.put("examdateList", examdateList);
