...
<c:if test="${not empty ts }">${ts }<%session.removeAttribute("ts"); %></c:if>      
      ...
      <li>
        <div class="arrow_box">
          ...
          <ul class="textinfo">
          <c:choose>
          	<c:when test="${not empty f.admin  }"><img src="images/admin.jpg" style="border-radius:50%"></c:when>
          	<c:otherwise><img src="images/${f.student.image }" style="border-radius:50%"></c:otherwise>
          </c:choose>
	...
	function changepage(page){
      ...
      <div class="toppic">
        ...
        <ul>
          ...
          <li style="font-size: 18px;"><a href="findVideoByIdplay.action?vid=${v.id}"><img src="video/images/${v.image}" width="80px;" height="80px">${v.vname }
            <p>${v.subject.sname }</p>
            </a></li>
            </c:forEach>
