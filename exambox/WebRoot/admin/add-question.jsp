...
<script type="text/javascript">
	...
	function checkzsd(){
		...
		var obj=document.getElementById("point-to-select");
		var tsobj = document.getElementById("error_zsd");
		
		if(obj.options.length==0){
			
			tsobj.innerHTML="你没有选择知识点";
			return false;
		}else{
		...
		function checkContent(){
			...
			var tsobj = document.getElementById("error_content");
			var info_str = document.getElementById("area_fir").value;
 			if (info_str=="") {
 				tsobj.innerHTML = "请输入试题内容";
 				tsobj.className = "cw";
 				return false;
 			}
 			else {
		...
		function checkjx(){
			var tsobj=document.getElementById("error_jx");
			var info_str=document.getElementById("area_second").value;
			if(info_str==""){
				tsobj.className="cw";
				tsobj.innerHTML="请输入题目解析";
				return false;
			}else{
				tsobj.innerHTML="";
				tsobj.className="cg";
		...
		var tsobj = document.getElementById("error_choose");
		var a =document.getElementsByName("question.optiona");
		var b =document.getElementsByName("question.optionb");
		var c = document.getElementsByName("question.optionc");
		var d = document.getElementsByName("question.optiond");
		
		if(a.length==0){
		alert("a")
			tsobj.className="cw";
			tsobj.innerHTML="请输入选项内容";
			return false;
		}
		if(b.length==0){
			alert("b")
	...
	function checkAdd(){
		...
		if(type=="2"){
			var group = document.getElementsByName("mutiquestion");
			var flage = 0;
        	...
        	if(flage==0){
        		var tsobj = document.getElementById("error_checkBox");
        		tsobj.className="cw";
        		tsobj.innerHTML="你需要选择正确选项";
        		return false;
        	}else{
        		tsobj.innerHTML="";
        	}
		...
		zk();
		if(checkzsd()&&checkjx()&&checkContent()){
			alert("输入正确")
			return true;
		}else{
			alert("输入格式错误")
				...
				<div class="col-xs-9">
					...
					<div class="page-content row">
						<form id="question-add-form" style="margin-top:40px;"
							action="addQuestion.action" method="post" onsubmit="return checkAdd()">
							...
							<div class="form-line question-knowledge">
								...
								<span class="form-message" id="error_zsd"></span>
							</div>

							<div class="form-line question-content">
								<span class="form-label"><span class="warning-label">*</span>试题内容：</span>
								<textarea class="add-question-ta" name="question.qtitle" id="area_fir" onblur="checkContent()"></textarea>
								<span class="add-img add-content-img" style="width:100px;"
									name="question.image">添加图片</span> <span class="form-message" id="error_content"></span>
							</div>
							<div class="form-line form-question-opt" style="display: block;">
								...
								<span class="form-message" id="error_choose"></span>
							</div>
							...
							<div class="form-line form-question-answer-muti correct-answer"
								...
								</span> <span class="form-message" id="error_checkBox"></span>
							</div>
							...
							<div class="form-line form-question-analysis"
								...
								<textarea class="add-question-ta" name="question.analysis" id="area_second" onblur="checkjx()"></textarea>
								<span class="form-message"  id="error_jx"></span> <br>
